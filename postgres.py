...
def connect(
    password, host="0.0.0.0", database="ImageDescribePipeDB", user="postgres", port=2000
):
...
def add_defining_image(
    conn, user_email, image_url, who_is_in, model=DeepFace.build_model("Facenet512")
):
...
def evaluate_image(
    conn, user_email, image_url, model=DeepFace.build_model("Facenet512")
):
    """
    ...
    os.system("mkdir ./tmp/ ./output/ ; touch ./tmp/img.jpg")
    ...
    try:
        ...
        for indx, face_info in enumerate(faces.values()):
            ...
            analyze_emotion(
                face_image, output_path, person_name, actions=["emotion", "gender"]
            )
